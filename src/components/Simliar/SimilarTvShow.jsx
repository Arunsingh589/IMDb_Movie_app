...
const SimilarTvShow = () => {
    ...
    const getData = () => {
        fetch(`https://api.themoviedb.org/3/tv/${id}/similar?api_key=8ba57bb67fe7f61dc132863540f5aa83&language=en-US`)
            .then(res => res.json())
            .then(data => {
                // Filter out movies without images
                const filteredMovies = data.results.filter(tv => tv.poster_path);
                setTvList(filteredMovies);
                setIsLoading(false);
            });
    };
    ...
    return (
        <div className="container mx-auto px-4 mt-20 relative">
            <h2 className="text-xl md:text-xl lg:text-3xl text-white font-bold mb-8 text-center">Similar Tv Show</h2>
